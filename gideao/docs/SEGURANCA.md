...
1. **Vetores de embedding ficam em claro.** Eles são necessários para o cálculo de
   ...
   conseguir lê-los. Se isso incomoda, use `GIDEAO_EMBEDDINGS=local` (nenhum texto
   sai da máquina) e mantenha o arquivo em disco com criptografia do sistema
   operacional (LUKS, FileVault, BitLocker).
2. **Enquanto o Gideão está rodando, as chaves estão na memória do processo.** É o
   que permite a ele responder. Um invasor com acesso de root à máquina *ligada e
   destrancada* alcança essas chaves. Nenhuma criptografia de aplicação resolve
   isso — a defesa é senha de tela, disco cifrado e `gideao panico` quando sair.

## Busca sem descriptografar

Buscar por palavra-chave normalmente exigiria abrir todos os registros. O Gideão usa
...
| Alguém copiou o `gideao.db` do backup do sistema | **Sim** |
| Google/funcionário do Google lendo o backup | **Sim** — sobe cifrado |
| Invasor com acesso ao Drive | **Sim** — só pega bytes cifrados |
| Alguém com a máquina ligada e destrancada | **Não** — use `gideao panico` e bloqueio de tela |
| Malware com root enquanto o Gideão roda | **Não** — as chaves estão na RAM |
...
Este último merece uma frase: o Gideão manda para a API da Anthropic o que for
...
- Todo acesso à UI e à API local exige o token de `GIDEAO_ACCESS_TOKEN`.
