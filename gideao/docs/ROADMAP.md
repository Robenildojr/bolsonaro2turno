...
| 13 | Anexos: imagens e PDFs na tela e no WhatsApp | `src/core/agent/attachments.ts` |
| 14 | Revisão de segurança adversarial — seis correções | `test/seguranca.test.ts` |
| 15 | Gideão: nome, persona masculina e informal, voz | `src/core/agent/prompt.ts`, `web/src/voice.js` |
| 16 | Engrenagem e painel de ajustes | `src/core/settings/`, `web/src/ajustes.js` |
| 17 | Ajuste por conversa | `src/tools/ajustes.tools.ts` |
| 18 | Memória de nascimento | `seed/genese.json`, `src/core/memory/genese.ts` |
| 19 | Atualização contínua: conhecimento, modelo, código | `src/core/updates/` |

**Todas as vinte concluídas.** 275 testes automatizados, 43 ferramentas, build
limpo e verificação de ponta a ponta com o sistema no ar, inclusive a interface
em Chromium real.
