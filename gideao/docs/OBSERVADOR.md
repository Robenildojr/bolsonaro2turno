...
Você pediu que ele acompanhasse o que você faz, para funcionar como segundo
...
sobrevive é o que virou memória — o objetivo é ele te entender, não manter um
dossiê.
...
```bash
npm run gideao -- observador ligar
```
...
```bash
npm run gideao -- observador estado
npm run gideao -- observador pausar 30
npm run gideao -- observador desligar
npm run gideao -- observador apagar    # apaga tudo que foi capturado
```
...
Quando falta alguma ferramenta, ele diz qual instalar em vez de ficar em
...
E a mitigação mais honesta: **você não precisa disto para o Gideão ser útil.** A
...
pensar "ele teria respondido melhor se soubesse o que eu estava fazendo às 15h",
