...
before(async () => {
  tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gideao-backup-'));
...
describe('coleta do backup', () => {
  it('reúne tudo que importa', () => {
    const conteudo = coletar(store, vault, { assistente: 'Gideão', dono: 'Robenildo' });
  ...
  it('resume de forma legível', () => {
    const resumo = resumirBackup(coletar(store, vault, { assistente: 'Gideão', dono: 'R' }));
...
describe('pacote cifrado', () => {
  it('abre com a senha certa e devolve o mesmo conteúdo', () => {
    const original = coletar(store, vault, { assistente: 'Gideão', dono: 'Robenildo' });
  ...
  it('recusa arquivo que não é backup do Gideão', () => {
...
describe('restauração em máquina nova', () => {
  it('abre só com a senha-mestra, sem o chaveiro original', () => {
    ...
    const pacote = empacotar(coletar(store, vault, { assistente: 'Gideão', dono: 'Robenildo' }), SENHA);

    const outro = fs.mkdtempSync(path.join(os.tmpdir(), 'gideao-novo-'));
    try {
...
describe('nome do arquivo', () => {
  it('é ordenável por data e não tem caractere problemático', () => {
    ...
    assert.match(nome, /^gideao-2026-09-14T18-30-45\.gideao$/);
    assert.ok(!/[:*?"<>|]/.test(nome));
  });
