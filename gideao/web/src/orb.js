 ...
 *   4. pontos      — as partículas que orbitam no meio enquanto ele pensa/responde
 *
 * A diferença entre os estados não é só de cor: no repouso existe um anel
 * nítido, e no momento em que ele responde o anel se dissolve num disco sem
 * contorno com pontos dentro — que foi exatamente o que você descreveu.
 *
 * Nada aqui depende de biblioteca. É canvas 2D puro.
 */

/** Centro da escala acima. O ajuste de cor é medido a partir daqui. */
const MATIZ_PADRAO = 258;
...
export class Orbe {
  ...
  definirEstado(estado) {
    ...
    if (estado !== this.estado) {
      this.estado = estado;
      this.alvo = this.comMatiz(ESTADOS[estado]);
      if (estado === 'thinking' || estado === 'responding') this.emitirOnda(0.5);
    }
  }

  /**
   * Muda a cor do orbe inteiro, dos ajustes.
   *
   * Cada estado tem a sua matiz — parado é mais azul, respondendo é mais
   * violeta — e essa diferença é o que faz dar para saber o que ele está
   * fazendo de canto de olho. Então o ajuste **desloca** a escala toda em vez
   * de pintar tudo de uma cor só: as distâncias entre os estados continuam as
   * mesmas, e a paleta inteira gira junto.
   */
  definirMatiz(base) {
    const n = Number(base);
    if (!Number.isFinite(n)) return;
    this.deslocamento = n - MATIZ_PADRAO;
    this.alvo = this.comMatiz(ESTADOS[this.estado] ?? ESTADOS.idle);
  }

  comMatiz(cfg) {
    const d = this.deslocamento ?? 0;
    return d ? { ...cfg, matiz: cfg.matiz + d } : cfg;
  }
   ...
   * — é o que faz parecer que ele está pensando, e não tocando uma animação.
   */
