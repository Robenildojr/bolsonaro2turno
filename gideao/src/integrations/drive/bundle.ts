 ...
 *
 *   GIDEAOBK  (8) | versão (1) | reservado (1) | sal (32) | IV (12) | tag (16) | ciphertext
 *                                                                              └ gzip(JSON)
 *
...
const MAGIC = Buffer.from('GIDEAOBK', 'ascii');
const VERSAO = 1;
const SAL_LEN = 32;
const IV_LEN = 12;
const TAG_LEN = 16;
...
function chaveDoArquivo(senha: string, sal: Buffer): Buffer {
  ...
  const chave = deriveSubkey(kek, 'gideao:backup-file:v1');
  kek.fill(0);
  return chave;
}
...
export function desempacotar(pacote: Buffer, senha: string): ConteudoBackup {
  ...
  if (!pacote.subarray(0, MAGIC.length).equals(MAGIC)) {
    throw new Error('isto não parece um backup do Gideão');
  }
