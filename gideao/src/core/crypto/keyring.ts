...
const CHECK_PLAINTEXT = 'gideao:keyring:ok:v1';

/**
 * Parâmetros padrão: N = 2^17 → ~134 MB de memória e ~1 s num laptop atual.
 * `GIDEAO_KDF_N` permite baixar o custo em máquinas fracas (e nos testes), com
 * piso em 2^14 para não virar uma senha decorativa.
 */
function defaultN(): number {
  const raw = Number(process.env.GIDEAO_KDF_N);
...
export class Keyring {
  ...
  create(passphrase: string, opts: { hint?: string } = {}): void {
    ...
    const data: KeyringFile = {
      version: 1,
      kdf,
      wrapped: seal(kek, root, 'gideao:keyring:wrap').toString('base64'),
      check: seal(deriveSubkey(root, 'gideao:check:v1'), CHECK_PLAINTEXT, 'gideao:keyring:check').toString('base64'),
  ...
  unlock(passphrase: string): void {
    ...
    let root: Buffer;
    try {
      root = openSealed(kek, Buffer.from(data.wrapped, 'base64'), 'gideao:keyring:wrap');
    } catch {
      kek.fill(0);
    ...
    const check = openSealed(
      deriveSubkey(root, 'gideao:check:v1'),
      Buffer.from(data.check, 'base64'),
      'gideao:keyring:check',
    );
  ...
  changePassphrase(current: string, next: string): void {
    ...
    const updated: KeyringFile = {
      ...data,
      kdf,
      wrapped: seal(kek, this.rootKey(), 'gideao:keyring:wrap').toString('base64'),
  ...
  key(purpose: 'data' | 'index' | 'backup' | 'vault' | 'audit'): Buffer {
    ...
    const derived = deriveSubkey(this.rootKey(), `gideao:${purpose}:v1`);
  ...
  backupKeyFromPassphrase(passphrase: string, salt: Buffer): Buffer {
    ...
    const kek = deriveKek(passphrase, kdf);
    const key = deriveSubkey(kek, 'gideao:backup-file:v1');
    kek.fill(0);
    return key;
  }
