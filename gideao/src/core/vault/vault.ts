 ...
 * vez, ela fica guardada, e o Gideão usa sozinho nas próximas vezes — sem
...
export interface VaultMeta {
  ...
  username?: string;
  /** Rótulos livres para o Gideão achar a credencial certa. */
  tags?: string[];
}
    ...
    const walk = (v: unknown, depth: number): unknown => {
      ...
      if (typeof v === 'string') {
        return v.replace(SECRET_REF, (_m, name: string) => {
          ...
          if (secret === null) {
            throw new Error(
              `credencial "${clean}" não existe no cofre — guarde com: gideao cofre set ${clean}`,
            );
          }
          used.add(clean);
