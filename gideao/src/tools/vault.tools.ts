...
const guardarCredencial: ToolDefinition<{
  ...
  async run(input) {
    ...
    // permite a ele lembrar, na próxima consulta, que já tem o acesso.
    ...
    return {
      ok: true,
      content: `Guardei no cofre como "${input.nome}". A partir de agora eu uso sozinho, você não precisa informar de novo. O valor não fica na conversa.`,
    };
  },
};
