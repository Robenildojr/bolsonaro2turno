...
const AJUDA = `
Gideão — agente pessoal

  gideao setup                          instalação (senha-mestra, banco, autorizações)
  gideao status                         estado geral do sistema
  gideao conversar "texto"               uma pergunta pelo terminal

  gideao memoria listar [tipo]          lista o que ele lembra
  gideao memoria buscar "termo"         busca na memória
  gideao memoria esquecer <id>          apaga uma memória
  gideao memoria consolidar             roda agora a rotina noturna
  gideao memoria reindexar              recalcula os embeddings
  gideao memoria semear [arquivo]     importa a memória de nascimento

  gideao permissoes                     lista autorizações
  gideao permissoes conceder <cap> <escopo>
  gideao permissoes revogar <id>
  gideao permissoes revogar-tudo
  gideao permissoes capacidades         lista o catálogo

  gideao cofre listar                   credenciais guardadas (sem valores)
  gideao cofre set <nome>               guarda uma credencial (pergunta no terminal)
  gideao cofre remover <nome>

  gideao observador [estado|ligar|desligar|pausar N|apagar]

  gideao backup agora [--sem-drive]     gera um backup cifrado
  gideao backup listar                  backups locais e no Drive
  gideao backup autorizar               conecta o Google Drive
  gideao backup restaurar <arquivo>     restaura de um backup

  gideao auditoria [--hoje] [--limite N] [--acao prefixo]
  gideao senha                          troca a senha-mestra
  gideao panico                         revoga tudo e tranca as chaves
  gideao manutencao                     compacta o banco e limpa vencidos
`;
...
async function main(): Promise<void> {
  ...
  if (!isInstalled()) {
    console.error('\n  O Gideão ainda não foi instalada neste perfil. Rode: npm run setup\n');
    process.exit(1);
  }
  ...
  keyring.unlock(process.env.GIDEAO_PASSPHRASE ?? (await askSecret('Senha-mestra: ')));
...
async function dispatch(comando: string, args: string[]): Promise<void> {
  ...
  switch (comando) {
    ...
    case 'conversar': {
      ...
      if (!texto) return console.log('  Use: gideao conversar "sua pergunta"');
...
async function memoriaCmd(args: string[], quando: (ts: number | null) => string): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'buscar': {
      ...
      if (!termo) return console.log('  Use: gideao memoria buscar "termo"');
    ...
    case 'esquecer': {
      const id = rest[0];
      if (!id) return console.log('  Use: gideao memoria esquecer <id>');
    ...
    case 'semear': {
      const { semear } = await import('../core/memory/genese.js');
      const r = await semear(args[1] || undefined);
      console.log(`\n  ✓ ${r.gravadas} memórias de nascimento importadas de ${r.arquivo}\n`);
      for (const a of r.assuntos) console.log(`    · ${a}`);
      console.log('');
      return;
    }
    ...
    default:
      console.log('  Subcomandos: listar, buscar, esquecer, consolidar, reindexar, perfil, semear');
  }
}
...
async function permissoesCmd(args: string[], quando: (ts: number | null) => string): Promise<void> {
  ...
  switch (sub) {
    case 'listar': {
      ...
      if (grants.length === 0) {
        return console.log('\n  Nenhuma autorização gravada. Ele vai perguntar em cada ação.\n');
      }
      console.log('');
    ...
    case 'conceder': {
      ...
      if (!cap || !escopo) return console.log('  Use: gideao permissoes conceder <capacidade> <escopo>');
      if (!CAPABILITIES[cap]) {
        return console.log(`  Capacidade desconhecida. Veja: gideao permissoes capacidades`);
      }
      broker.grant(cap, escopo, 'concedido pela linha de comando');
      console.log(`\n  ✓ ${cap} liberado em ${escopo}. Ele não vai mais perguntar.\n`);
      return;
    }

    case 'negar': {
      const [cap, escopo] = rest;
      if (!cap || !escopo) return console.log('  Use: gideao permissoes negar <capacidade> <escopo>');
      broker.deny(cap, escopo, 'negado pela linha de comando');
      console.log(`\n  ✓ ${cap} negado em ${escopo}. Ele nem vai perguntar.\n`);
      return;
    }

    case 'revogar': {
      const id = rest[0];
      if (!id) return console.log('  Use: gideao permissoes revogar <id>');
...
async function cofreCmd(args: string[], quando: (ts: number | null) => string): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'set': {
      const nome = rest[0];
      if (!nome) return console.log('  Use: gideao cofre set <nome>');
    ...
    case 'remover': {
      const nome = rest[0];
      if (!nome) return console.log('  Use: gideao cofre remover <nome>');
...
async function backupCmd(args: string[], quando: (ts: number | null) => string): Promise<void> {
  ...
  switch (sub) {
    case 'agora': {
      const senha = process.env.GIDEAO_PASSPHRASE ?? (await askSecret('  Confirme a senha-mestra: '));
    ...
    case 'restaurar': {
      ...
      if (!arquivo) return console.log('  Use: gideao backup restaurar <caminho do arquivo .gideao>');
