 ...
 * pergunta o mínimo necessário para o Gideão já servir para alguma coisa no
 * primeiro uso. O resto se configura conversando com ele depois.
 */
...
export async function runSetup(): Promise<void> {
  ...
  const assistente = await ask(`  E o nome dele? [${cfg.assistantName}] `);
  ...
  console.log('  não for liberado agora, ele pede na hora — e uma vez que você');
  console.log('  responder "sempre", não pergunta de novo.\n');

  const pastaTrabalho = await ask(
    '  Qual pasta ele pode ler e escrever livremente? (vazio = nenhuma por enquanto)\n  Ex.: ~/Documentos/processos\n  > ',
  );
  ...
  console.log('  quando você precisar. Ele pede na hora, com contexto.\n');

  // ── 8. memória de nascimento ───────────────────────────────────────────────
  console.log(`${linha}\n  Memória de nascimento\n${linha}\n`);
  console.log('  Ele pode começar já sabendo quem é você, como você quer ser');
  console.log('  tratado e por que cada peça do sistema é como é — tudo que ficou');
  console.log('  estabelecido na conversa em que ele foi construído.\n');
  console.log('  Sem isso ele funciona igual, só começa do zero e leva algumas');
  console.log('  semanas de conversa para chegar no mesmo lugar.\n');

  if (await confirm('  Importar a memória de nascimento?', true)) {
    try {
      // A memória não é iniciada pelo assistente de instalação — ela só é
      // necessária a partir daqui.
      const { initMemory } = await import('../core/memory/index.js');
      initMemory(store, loadConfig({ reload: true }));
      const { semear } = await import('../core/memory/genese.js');
      const r = await semear();
      console.log(`  ✓ ${r.gravadas} memórias importadas.\n`);
    } catch (err) {
      console.log(`  ✗ Não consegui importar: ${describeError(err)}`);
      console.log('    Dá para fazer depois com: npm run gideao -- memoria semear\n');
    }
  }
  ...
  console.log('\n  Guarde esse endereço. Ele está salvo em ~/.gideao/config.json.\n');
}
