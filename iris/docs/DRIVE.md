...
perder inteiro — disco, keyring, tudo — você instala a Íris num computador novo
...
| | Local (`~/.iris/backups`) | Google Drive |
|---|---|---|
...
Esse escopo dá acesso **apenas aos arquivos que a própria Íris criar**. Ela não
...
É o escopo mínimo que serve. Se algum dia a Íris pedir mais que isso, desconfie.

## Usar

```bash
npm run iris -- backup agora              # gera e envia
npm run iris -- backup agora --sem-drive  # só a cópia local
npm run iris -- backup listar             # o que existe
npm run iris -- backup restaurar ~/.iris/backups/iris-2026-09-14T18-30-45.iris
```
...
A cada 6 horas, **se** `IRIS_PASSPHRASE` estiver no ambiente — a chave do pacote
vem da senha-mestra, e sem ela a Íris não tem como gerar o arquivo sozinha.

Esse é um trade-off real e você decide qual lado prefere:

- **com `IRIS_PASSPHRASE` no `.env`**: backup automático funciona, e a senha
...
```bash
npm run iris -- backup restaurar <arquivo>
```

Ela mostra o que o pacote contém, pede confirmação e **acrescenta**.
...
npm run iris -- backup restaurar ~/Downloads/iris-2026-09-14T18-30-45.iris
```
