...
Isso não é conversa fiada: a cada 4 turnos ela extrai o que aprendeu, e é assim
que o retrato dela sobre você começa a existir.
...
Ela consulta na base pública do CNJ, guarda quem é o Almeida, e passa a avisar
...
Ela marca e avisa com antecedência (audiência: um dia; prazo: dois dias).
A partir daqui a agenda entra no contexto de toda conversa — ela passa a saber o
que está marcado antes de responder qualquer coisa sobre o seu dia.

## Dia 4 — a primeira credencial

Quando ela precisar entrar num sistema, vai pedir autorização. Guarde a senha
...
A partir daí ela usa sozinha. Você não informa de novo, e o valor nunca aparece
no diálogo — é substituído dentro da ferramenta, no último instante.

## Semana 2 — ampliar conforme a confiança

Olhe o que ela andou fazendo:

```bash
npm run iris -- auditoria --hoje
npm run iris -- memoria perfil
```

O `memoria perfil` é o mais revelador: é o retrato que ela montou de você a
partir das conversas. Se estiver bom, ela entendeu. Se estiver genérico, é
porque ainda falta conversa — ou porque você não corrigiu quando ela errou.
...
```bash
npm run iris -- permissoes conceder arquivo.ler '/home/eu/**'
npm run iris -- permissoes conceder shell.executar 'git'
```
...
a pena não perder. Antes disso, a cópia local em `~/.iris/backups` já resolve.
...
**Você não precisa dele para a Íris ser útil.** A memória das conversas é o
motor; o observador só acrescenta contexto passivo.

Rode algumas semanas sem. Se em algum momento você pensar "ela teria respondido
...
- Ela começa a responder sem você precisar dar contexto.
- Ela lembra da preferência que você mencionou de passagem três semanas atrás.
- O aviso de audiência chega antes de você lembrar sozinho.
- Você para de explicar quem é cada cliente.

## Sinais de que algo está errado

- **Ela responde genérico.** Falta conversa, ou você não corrigiu os erros.
  Rode `memoria perfil` e veja o que ela acha que sabe.
- **Ela pergunta autorização toda hora.** Você respondeu "agora" onde devia ter
  respondido "sempre aqui". Veja com `permissoes` e conceda o que faltar.
- **Ela inventa.** Isso é um problema sério e não deve ser tolerado — traga o
  caso concreto, corrija na conversa, e veja em `auditoria` se a consulta que
  ela diz ter feito aconteceu mesmo.
