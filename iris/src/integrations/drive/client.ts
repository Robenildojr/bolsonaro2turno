 ...
 * arquivos que a própria Íris criou. Ela não consegue ler, listar ou apagar
...
export class DriveClient {
  ...
  async autorizar(): Promise<void> {
    ...
    getVault().set('drive.refresh_token', tokens.refresh_token, {
      kind: 'token',
      meta: { service: 'drive.google.com', description: 'Backup cifrado da memória da Íris' },
    });
  ...
  private receberCodigo(
    ...
    return new Promise((resolve, reject) => {
      ...
      servidor.listen(0, '127.0.0.1', () => {
        ...
        console.log('  Escopo pedido: apenas os arquivos que a Íris criar. Nada mais do seu Drive.\n');
      });
  ...
  private async token(): Promise<string> {
    ...
    if (!refresh) throw new Error('o Drive ainda não foi autorizado. Rode: iris backup autorizar');
    ...
    if (!res.ok) {
      throw new Error(
        `não consegui renovar o acesso ao Drive (${res.status}). ` +
          'Se você revogou a permissão, autorize de novo: iris backup autorizar',
      );
    }
  ...
  /** Acha (ou cria) a pasta da Íris. Devolve o id. */
  ...
  async enviar(pastaId: string, nome: string, conteudo: Buffer): Promise<ArquivoDrive> {
    const token = await this.token();
    const limite = `iris${randomBytes(12).toString('hex')}`;
...
const PAGINA_OK = `<!doctype html><meta charset="utf-8"><title>Íris</title>
...
const PAGINA_ERRO = (erro: string) => `<!doctype html><meta charset="utf-8"><title>Íris</title>
