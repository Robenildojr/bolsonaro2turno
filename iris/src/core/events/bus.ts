...
type Handler<K extends keyof IrisEvents> = (payload: IrisEvents[K]) => void | Promise<void>;
...
class TypedBus {
  ...
  on<K extends keyof IrisEvents>(event: K, handler: Handler<K>): () => void {
    const wrapped = (payload: IrisEvents[K]) => {
  ...
  once<K extends keyof IrisEvents>(event: K, handler: Handler<K>): void {
    this.emitter.once(event as string, (p: IrisEvents[K]) => {
      void Promise.resolve(handler(p)).catch(() => {});
    });
  }

  emit<K extends keyof IrisEvents>(event: K, payload: IrisEvents[K]): void {
