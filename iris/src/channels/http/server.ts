...
function tokenOf(req: FastifyRequest | { url?: string; headers: Record<string, unknown> }): string | null {
  ...
  if (headers['x-iris-token']) return headers['x-iris-token']!;
  ...
  if (cookie) {
    const match = /(?:^|;\s*)iris_token=([^;]+)/.exec(cookie);
...
<div>
  ...
  <p>Abra o endereço que apareceu no terminal ao iniciar a Íris, ou consulte-o com
     <code>npm run iris -- status</code>.</p>
</div>`;
