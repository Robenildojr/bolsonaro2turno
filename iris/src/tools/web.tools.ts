 ...
 *    É por elas que ela "aprende com a internet" no dia a dia.
...
const USER_AGENT =
  'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/131.0 Safari/537.36 Iris/1.0';
...
const salvarNoCache: ToolDefinition<{ nome: string; conteudo: string }> = {
  ...
  description:
    'Guarda um texto na pasta de trabalho da Íris para usar depois — minuta, resultado de consulta, anotação longa. Não exige autorização porque escreve só na própria pasta dela.',
  schema: {
    type: 'object',
    additionalProperties: false,
