 ...
 *
 *  1. o próprio cofre da Íris é intocável — `keyring.json` e `iris.db` não são
...
function guard(absolute: string): void {
  if (isProtectedPath(absolute)) {
    throw new Error(
      'este arquivo é material criptográfico da própria Íris (ou credencial do sistema) e não é acessível por ferramenta',
    );
  }
}
