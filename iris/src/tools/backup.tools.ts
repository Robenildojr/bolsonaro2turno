 ...
 *
 * Ela consegue **disparar** o backup, mas não consegue gerar um sem a
 * senha-mestra — que ela não tem. Se a senha não estiver no ambiente, a
...
const fazerBackup: ToolDefinition<{ enviar_ao_drive: boolean }> = {
  ...
  timeoutMs: 600_000,
  async run(input) {
    const senha = process.env.IRIS_PASSPHRASE;
    if (!senha) {
      return {
        ok: false,
        content:
          'Não consigo gerar o backup sozinha: a chave do pacote é derivada da senha-mestra, e eu ' +
          'não a tenho em memória. O dono pode rodar `npm run iris -- backup agora` no terminal, ' +
          'ou deixar IRIS_PASSPHRASE no ambiente para o backup automático funcionar.',
      };
    }
